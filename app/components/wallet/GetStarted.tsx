 ...
 *
 * Two things are missing at that point and they are not alike. Native STT for
 * gas cannot be minted; it comes from faucets we do not run, so that is a link
 * out. Test collateral is mintable by anyone - `faucet(uint256)` is public on
 * the testnet tUSDC contract - so that is a button.
 *
 * Gas comes first, and the mint button stays disabled until it is there,
 * because minting is itself a transaction. Offering it to a wallet with no STT
 * walks the visitor straight into the trap this panel exists to prevent: the
 * node rejects the write for want of a reserve and calls it a parameter
 * error.
 *
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded border border-ink-700 bg-ink-950/60 p-4">
          <div className="flex items-baseline justify-between gap-2">
            <h3 className="text-[13px] font-semibold text-gray-200">1. STT for gas</h3>
        ...
        <div className="rounded border border-ink-700 bg-ink-950/60 p-4">
          <div className="flex items-baseline justify-between gap-2">
            <h3 className="text-[13px] font-semibold text-gray-200">
              2. {COLLATERAL_SYMBOL} to trade with
            </h3>
            <span className="mono text-[11px] text-gray-500">
          ...
          <p className="mt-1.5 text-[12.5px] leading-relaxed text-gray-500">
            {!needsCollateral
              ...
              : needsGas
                ? "Get gas first - minting is itself a transaction, and it needs the same reserve."
          ...
          {needsCollateral && (
            <button
              type="button"
              onClick={mint}
              disabled={minting || !walletClient || needsGas}
